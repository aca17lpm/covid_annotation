...
class ClassTopicModel(nn.Module):
    def __init__(self, vocab_dim, config, cls_lambda=10):
        ...
        hidden_dim = 300
        ntopics = 50
        n_class_topics = 50+self.n_classes

        self.loss_weight_lambda = float(math.ceil(vocab_dim/self.n_classes))*cls_lambda
        ...
        self.topic_hidden = WVHidden(n_class_topics, ntopics)
    ...
    def forward(self,x, mask=None, n_samples=1, bow=None, train=False, true_y=None, pre_embd=False, true_y_ids=None):
        ...
        if train:
            ...
            for i in range(n_samples):
                ...
                log_y_hat = torch.log_softmax(y_hat, dim=-1)
                log_y_hat_z = torch.cat((z, log_y_hat), dim=-1)
                log_y_hat_z_h = self.topic_hidden(log_y_hat_z)
                log_prob = self.topics(log_y_hat_z_h)
                ...
                #class_loss += self.class_criterion(y_hat, true_y_ids)
            rec_loss = rec_loss / n_samples
            class_loss = class_loss / n_samples
            class_topic_loss = class_topic_loss / n_samples


            minus_elbo = rec_loss + kld + class_topic_loss + self.loss_weight_lambda*class_loss
