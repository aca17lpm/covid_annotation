...
from WvMLLib import WVvaeHANDataIter as WVdataIter
from WvMLLib import ModelUltiHANVAEtm as modelUlti
from WvMLLib import ReaderPostProcessorHanVAE as ReaderPostProcessor
from WvMLLib.models import HANvae as NVDM
...
if __name__ == "__main__":
    ...
    postProcessor = ReaderPostProcessor(tokenizer='bert', config=config, word2id=True, remove_single_list=False, x_fields=['Claim', 'Explaination'], max_sent_len=60, max_doc_len=20, add_spec_tokens=True, use_source=True)
    trainDataIter = WVdataIter(merged_json_file, postProcessor=postProcessor, config=config, shuffle=True)
    #print(next(trainDataIter))
    #print(next(trainDataIter))
    #print(next(trainDataIter))
    ...
    common_dictionary.filter_extremes(no_below=3, no_above=0.8, keep_n=3000)
    ...
    for each_fold in kf.split(all_ids):
        ...
        mUlti.train(trainBatchIter, cache_path=fold_cache_path, num_epohs=220, valBatchIter=testBatchIter)
